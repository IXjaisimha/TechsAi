...
function ruleMatch(resume, jobNormal, jobHidden, jobDetails = {}) {
  ...
  const normal = (jobNormal || []).map(s => ({ name: (s.skill_name || '').toLowerCase(), weight: s.weight || 5, required_level: s.required_level }));
  const hidden = (jobHidden || []).map(s => ({ name: (s.skill_name || '').toLowerCase(), importance: s.importance || 7 }));
  ...
  const overall_score = Math.round(
    technical_skills_score * 0.50 + 
    hidden_criteria_score * 0.25 + 
    experience_score * 0.15 +
    50 * 0.10 // Soft skills baseline
  );

  const match_grade = overall_score >= 80 ? 'Excellent' : overall_score >= 65 ? 'Good' : overall_score >= 50 ? 'Fair' : 'Poor';
...
async function matchResumeToJob(resumeData, jobSkills, hiddenSkills, jobDetails = {}) {
  ...
  return {
    ...
    match_grade: data.match_grade || 'Fair',
    ...
    extra_skills: Array.isArray(data.extra_skills) ? data.extra_skills : [],
    ai_insights: data.ai_insights || { strengths: [], weaknesses: [], recommendations: [], red_flags: [], unique_selling_points: [] },
    hidden_match_analysis: data.hidden_match_analysis || {},
