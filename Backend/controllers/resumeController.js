...
exports.getAllResumes = async (req, res) => {
  try {
    ...
    const resumeSkills = await ResumeSkill.find({
      resume_id: { $in: resumeIds }
    });

    // 3. Merge data
    const mergedData = resumes.map(r => {
      const skillsData = resumeSkills.find(rs => rs.resume_id === r.resume_id);
      return {
        id: r.resume_id,
        filename: r.file_name,
        uploadedAt: r.created_at, // Mapping created_at to uploadedAt for frontend
        extractionSuccess: !!skillsData,
        extractedSkills: skillsData ? skillsData.skills : [],
        metadata: skillsData ? skillsData.metadata : {}
      };
    });
