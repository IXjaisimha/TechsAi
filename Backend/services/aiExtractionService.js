...
const extractFromResume = async (filePath) => {
  ...
  if (apiKey && apiKey.length > 10) {
    console.log("🤖 Attempting AI analysis (OpenRouter)...");
    try {
      const geminiResult = await extractWithGemini(text);

      if (geminiResult && geminiResult.data) {
        console.log(`✅ AI extraction completed (Model: ${geminiResult.model})`);
      ...
      // Fallback to regex will happen below
    }
  }
...
const extractSkillsFromJD = async (jobTitle, jobDescription) => {
  console.log("🔍 Extracting skills from JD...");
  const prompt = `Analyze this Job Description and extract technical skills and soft skills.
  ...
  {
    "technical_skills": [
      { "skill_name": "Java", "weight": 9, "level": "Advanced" },
      { "skill_name": "Spring Boot", "weight": 8, "level": "Intermediate" }
    ],
    "soft_skills": [
      { "skill_name": "Communication", "importance": 8 },
      { "skill_name": "Leadership", "importance": 9 }
    ]
  }
  
  Rules: 
  - Weight/Importance should be 1-10.
  - Infer level if possible, else "Intermediate".`;

  try {
    const result = await runGemini(prompt);
    if (result && result.data) {
      return {
        technical_skills: result.data.technical_skills || [],
